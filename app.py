...
- Simple field selection flow
- Strong helper functions and comments
- Preserved ML prediction logic
- Preserved SHAP explanation flow
- No custom CSS
"""
...
def get_investor_decision_text(decision: str) -> str:
    """
...
def get_threshold_note(threshold: float) -> str:
    """
    Return plain-English guidance for the currently selected decision threshold.
    """
    if threshold < 0.15:
        return "Very conservative: only the strongest borrower profiles are likely to pass."
    if threshold < 0.25:
        return "Conservative: balances approval volume with tighter default control."
    if threshold < 0.40:
        return "Moderate: allows more loans through while accepting higher expected risk."
    return "Aggressive: maximizes approvals, but expected default exposure becomes much higher."
...
def load_shap_explainer():
    ...
    """
    raw_model = joblib.load(os.path.join(MODEL_DIR, "xgb_with_tradeline.pkl"))
...
st.markdown(
    """
Use this dashboard to evaluate a borrower profile, estimate default risk,
and review the major factors that influence the model’s decision.
"""
)

st.caption(
    "This project presents a LendingClub-style credit risk workflow with interactive borrower inputs, "
    "model scoring, expected-value estimates, and local explanation support."
)
...
current_keys = [
    ...
    if key not in st.session_state.user_inputs
    or st.session_state.user_inputs.get(key) in [None, "", []]
]
...
threshold = st.slider(
    "Flag loans at or above this predicted default probability:",
    min_value=0.00,
    max_value=1.00,
...
st.caption(
    "Default threshold: 20%. This is a conservative investor-style cutoff that screens out loans "
    "once predicted default risk reaches one borrower in five."
)
st.caption(get_threshold_note(threshold))
...
if st.button("Run Prediction", disabled=(len(selected_keys) == 0), use_container_width=False):
    ...
    try:
        ...
        investor_decision = get_investor_decision_text(decision)
        ...
        # =====================================================
        st.subheader("Expected Return Lens")
        ...
        st.caption(
            f"Estimated using entered loan amount, entered interest rate, predicted default probability, "
            f"and an assumed {profit_info['lgd']:.0%} loss-given-default."
        )
        ...
        try:
            ...
            st.caption(
                "Red bars push predicted default risk higher. Green bars push predicted default risk lower."
            )
            ...
            st.info(
                "Detailed SHAP output is temporarily unavailable, so a fallback explanation is shown below."
            )
        ...
        if show_diagnostics:
            ...
            with st.expander("Transformed model input preview"):
